...
@section('title', 'Add Exam Result | ' . config('app.name'))

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-ink-900">Add Exam Result</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-ink-200/80 p-6">
        ...
        @endif
        <p class="mb-4 text-sm text-ink-500">Student ID must match <code class="bg-ink-100 px-1 rounded">students.id</code> in the Students module.</p>

        <form method="POST" action="{{ route('finance.exam-results.store') }}" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <select name="exam_id" id="exam_id" required class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
                        ...
                        @foreach($exams as $e)
                            <option value="{{ $e->id }}" {{ old('exam_id') == $e->id ? 'selected' : '' }}>{{ $e->name }} ({{ $e->term->name ?? '' }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="sm:col-span-2">
                    <label for="student_id" class="block text-sm font-medium text-ink-700 mb-1">Student ID <span class="text-red-500">*</span></label>
                    <input type="number" name="student_id" id="student_id" value="{{ old('student_id') }}" min="1" required class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
                </div>
                <div>
                    <label for="marks" class="block text-sm font-medium text-ink-700 mb-1">Marks <span class="text-red-500">*</span></label>
                    <input type="number" name="marks" id="marks" value="{{ old('marks') }}" step="0.01" min="0" required class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
                </div>
                <div>
                    <label for="grade" class="block text-sm font-medium text-ink-700 mb-1">Grade</label>
                    <input type="text" name="grade" id="grade" value="{{ old('grade') }}" class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500" placeholder="e.g. A, B+">
                </div>
                <div class="sm:col-span-2">
            ...
            <div class="flex gap-2">
                <button type="submit" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-accent-500 rounded-lg hover:bg-accent-600">Save result</button>
