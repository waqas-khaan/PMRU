...
@section('title', 'Add Fee Structure | ' . config('app.name'))

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-ink-900">Add Fee Structure</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-ink-200/80 p-6">
        ...
        <form method="POST" action="{{ route('finance.fee-structures.store') }}" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="class_name" id="class_name" value="{{ old('class_name') }}" class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500" placeholder="e.g. Class 1">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date') }}" class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
                </div>
            </div>
