...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-ink-500 mt-1">Manage terms per academic year.</p>
        </div>
        <a href="{{ route('finance.terms.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-accent-500 rounded-lg hover:bg-accent-600">Add term</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-ink-200/80 p-6">
        @if($terms->isEmpty())
            <p class="text-sm text-ink-500">No terms yet. Add an academic year first, then add terms.</p>
        @else
                        ...
                        @foreach($terms as $term)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-right space-x-2">
                                    <a href="{{ route('finance.terms.show', $term) }}" class="text-accent-600 hover:text-accent-700">View</a>
                                    <a href="{{ route('finance.terms.edit', $term) }}" class="text-accent-600 hover:text-accent-700">Edit</a>
                                    <form method="POST" action="{{ route('finance.terms.destroy', $term) }}" class="inline" onsubmit="return confirm('Delete this term?');">
                                        @csrf
                                        @method('DELETE')
