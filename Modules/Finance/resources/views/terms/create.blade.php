...
@section('title', 'Add Term | ' . config('app.name'))

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-ink-900">Add Term</h1>
        <a href="{{ route('finance.terms.index') }}" class="text-sm text-accent-600 hover:text-accent-700 font-medium">← Back to list</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-ink-200/80 p-6">
        ...
        <form method="POST" action="{{ route('finance.terms.store') }}" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500" placeholder="e.g. Term 1">
                </div>
                <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('finance.terms.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-ink-600 bg-ink-100 rounded-lg hover:bg-ink-200">Cancel</a>
            </div>
        </form>
    </div>
@endsection
