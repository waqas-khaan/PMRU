...
@section('title', 'Add Section | ' . config('app.name'))

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-ink-900">Add Section</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-ink-200/80 p-6">
        ...
        <form method="POST" action="{{ route('academics.sections.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500" placeholder="e.g. A">
            </div>
            <div>
                <label for="capacity" class="block text-sm font-medium text-ink-700 mb-1">Capacity</label>
                <input type="number" name="capacity" id="capacity" value="{{ old('capacity') }}" min="0" class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
            </div>
