...
@section('title', 'Add Student | ' . config('app.name'))

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-ink-900">Add Student</h1>
        ...
        <form method="POST" action="{{ route('students.store') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="address" id="address" rows="2" class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">{{ old('address') }}</textarea>
                </div>
                <div>
                    ...
                    @if(isset($schoolClasses) && $schoolClasses->isNotEmpty())
                        <select name="class" id="class" class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
                            ...
                            @foreach($schoolClasses as $c)
                                <option value="{{ $c->name }}" {{ old('class') === $c->name ? 'selected' : '' }}>{{ $c->name }}</option>
                            @endforeach
                        </select>
                    @else
                    ...
                    @if(isset($sections) && $sections->isNotEmpty())
                        ...
                        <select name="class_section" id="class_section" class="w-full px-3 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
                            ...
                            @foreach($sections as $sec)
                                <option value="{{ $sec->schoolClass->name ?? '' }}|{{ $sec->name }}" {{ old('class_section') === ($sec->schoolClass->name ?? '').'|'.$sec->name ? 'selected' : '' }}>{{ $sec->schoolClass->name ?? '?' }} – {{ $sec->name }}</option>
                            @endforeach
                        </select>
